#!/usr/bin/env python3
"""
Azure 900 Transcript Processor - Improved Version
...
def extract_key_concepts(text, video_title):
    ...
    for sentence in sentences:
        ...
        elif re.search(r'(is|are|means|refers to)', sentence, re.IGNORECASE):
            concepts.append({
                'question': f"What does {sentence.split()[0]} mean?",
                'context': sentence,
                'source': video_title
            })
        
        # Look for benefit patterns
        elif re.search(r'(benefit|advantage|feature)', sentence, re.IGNORECASE):
            concepts.append({
                'question': f"What are the benefits of {sentence.split()[0]}?",
                'context': sentence,
                'source': video_title
            })
    
    return concepts[:3]  # Limit to 3 concepts per video
...
def generate_flashcard_from_concept(concept, flashcard_id):
    ...
    if "Cloud" in source or "CapEx" in source or "OpEx" in source:
        category = "Cloud Concepts"
    elif "Security" in source or "Defender" in source or "Key Vault" in source or "RBAC" in source:
    ...
    elif "Virtual Machine" in source or "Storage" in source or "Network" in source or "Compute" in source:
        category = "Core Azure Services"
    
    # Generate better options based on question content
    if "What is" in question or "What does" in question:
        ...
        answer = "D"
    elif "Azure" in question:
        options = [
            "A) Microsoft's cloud platform",
            "B) Google's cloud platform",
            "C) Amazon's cloud platform",
...
def main():
    ...
    print("\nSample flashcards:")
    for i, flashcard in enumerate(unique_flashcards[:5]):
